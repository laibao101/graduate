...
router.get('/read', function(req, res, next) {
    ...
    fs.readFile(PATH + type + '.json', function(err, data) {
        if (err) {
            return res.send({status: 0, info: '读取文件出现异常'});
        }
        var COUNT = 50;
        //TODO: try
        var obj = [];
...
router.post('/write', function(req, res, next) {
    if (!req.session.user) {
        return res.send({status: 0, info: '未鉴权认证'});
    }
    //文件名
    var type = req.param('type') || '';
    //关键字段
    ...
    if (!type || !url || !title || !img) {
        return res.send({status: 0, info: '提交的字段不全'});
    }
    //1)读取文件
    ...
    fs.readFile(filePath, function(err, data) {
        ...
        fs.writeFile(filePath, newData, function(err) {
            ...
            return res.send({status: 1, info: obj});
        });
    });
});
...
router.post('/write_config', function(req, res, next) {
    ...
    fs.writeFile(PATH + 'config.json', newData, function(err) {
        if (err) {
            return res.send({status: 0, info: '写入数据失败'});
        }
        return res.send({status: 1, info: obj});
    });
});
